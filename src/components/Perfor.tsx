...
const weekData = [
  ...
  const updateChart = (data: { time: string, value: number }[]) => {
    if (chartRef.current) {
  ...
  return (
    <div>
      <div className="flex space-x-2 p-2 bg-black text-white">
        <button
          onClick={() => {
            setData(dayData);
            setActiveButton('1D');
          }}
          className={`px-4 py-2 rounded-md ${activeButton === '1D' ? 'bg-gray-600' : 'bg-gray-800'}`}
        >
          1D
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-md ${activeButton === '1W' ? 'bg-gray-600' : 'bg-gray-800'}`}
        >
          1W
        </button>
        {/* Add more buttons for other time ranges if needed */}
      </div>
      
      <div className='w-[900px] max-w-[1500px] max-h-[800px]'/>
